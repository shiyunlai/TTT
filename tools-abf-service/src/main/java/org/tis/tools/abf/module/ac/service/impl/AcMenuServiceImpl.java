...
public class AcMenuServiceImpl extends ServiceImpl<AcMenuMapper, AcMenu> implements IAcMenuService {
    ...
    public AcMenu moveMenu(String targetGuid, String moveGuid, BigDecimal order) throws AcMenuManagementException {
        try {
            ...
            // 改变显示顺序
            // 重新排序源菜单下的子菜单自减
            acMenuServiceExt.reorderMenu(sourceGuid, sourceOrder, REORDER_AUTO_MINUS);
            // 重新排序目标菜单下的子菜单自增
            acMenuServiceExt.reorderMenu(targetGuid, order, REORDER_AUTO_PLUS);
            // 更改移动的重组菜单信息
     ...
     */
    @Override
    public boolean deleteAllSubAcMenu(String menuGuid) throws AcMenuManagementException {
        try {
            ...
            if (guidList.size() > 0) {
                result = acMenuService.delete(new EntityWrapper<AcMenu>().in(AcMenu.COLUMN_GUID, guidList));
                // 其余兄弟菜单重新排序
                acMenuServiceExt.reorderMenu(parentGuid, index, "minus");
            }
            return result;
        } catch (Exception e) {
            e.printStackTrace();
     ...
     */
    @Override
    public boolean updateAcMenu(AcMenu acMenu) throws AcMenuManagementException {
        try {
            if (null == acMenu) {
            ...
            return acMenuService.updateById(acMenu);
        } catch (ToolsRuntimeException ae) {
            throw ae;
        } catch (Exception e) {
            e.printStackTrace();
     ...
     */
    @Override
    public boolean createChildMenu(AcMenu acMenu) throws AcMenuManagementException {
        try {
            if (null == acMenu) {
                throw new AcMenuManagementException(ACExceptionCodes.OBJECT_IS_NULL, wrap("AcMenu"));
            }
...
//            acMenu.setGuid(guid);
            ...
            return acMenuService.insert(acMenu);
        } catch (ToolsRuntimeException ae) {
            throw ae;
        } catch (Exception e) {
            e.printStackTrace();
     ...
     */
    @Override
    public boolean createRootMenu(AcMenu acMenu) throws AcMenuManagementException {
        try {
            if (null == acMenu) {
                throw new AcMenuManagementException(ACExceptionCodes.OBJECT_IS_NULL, wrap("AcMenu"));
            }
...
//            acMenu.setGuid(GUID.menu());
    ...
    public List<AcMenu> queryRootMenu(String GUID_APP) throws AcMenuManagementException {
        try {
            ...
            EntityWrapper wrapper = new EntityWrapper();
            wrapper.eq(AcMenu.COLUMN_GUID_APP, GUID_APP).isNull(AcMenu.COLUMN_GUID_PARENTS);
